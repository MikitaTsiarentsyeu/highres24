...
try:
    ...
    print("Файл не знойдзены")
    exit()

while True:
    max_len = input("Увядзіце максімальную колькасць сімвалаў на якія будзе разбіты тэкст (>20): ")
    if max_len.isdigit() and int(max_len) > 20:
        max_len = int(max_len)
        break
    else:
        print("Трэба цэлае лік, больш за 20!")
...
print("Гатова! Вынік запісаны ў файл output.txt")
